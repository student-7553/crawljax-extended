...
public class InMemoryStateFlowGraph implements Serializable, StateFlowGraph {
    ...
    /**
     * Adds a state (as a vertex) to the State-Flow Graph if not already present. More formally, adds
     * the specified vertex, v, to this graph if this graph contains no vertex u such that
     ...
     *
     * @param stateVertex the state to be added.
     * @param correctName if true the name of the state will be corrected according to the internal
     *                    state counter.
    ...
    /**
     * Adds the specified edge to this graph, going from the source vertex to the target vertex. More
     * formally, adds the specified edge, e, to this graph if this graph contains no edge e2 such that
     * e2.equals(e). If this graph already contains such an edge, the call leaves this graph unchanged
     * and returns false. Some graphs do not allow edge-multiplicity. In such cases, if the graph
     * already contains an edge from the specified source to the specified target, than this method
     * does not change the graph and returns false. If the edge was added to the graph, returns true.
    ...
    public ImmutableList<Eventable> getShortestPath(StateVertex start, StateVertex end) {
        ...
        try {
            return ImmutableList.copyOf(
                    DijkstraShortestPath.findPathBetween(sfg, start, end).getEdgeList());
        } finally {
            readLock.unlock();
        }
    }
